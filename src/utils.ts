...
export function parseLastAssistant(lastAssistant: unknown): LastAssistantInfo {
    ...
    if (typeof usage === 'object' && usage !== null) {
        ...
        if (typeof cost === 'object' && cost !== null) {
            ...
            if (typeof c.input === 'number' && typeof c.output === 'number' && typeof c.total === 'number') {
...
function normalizeMessage(raw: unknown): FormattedMessage[] {
    ...
    if (role === 'toolResult') {
        const content = typeof obj.content === 'string' ? obj.content : truncate(safeStringify(obj.content) ?? '', 2000)
        const toolCallId =
    ...
    for (const item of obj.content as unknown[]) {
        ...
        if (type === 'tool_result') {
            const resultContent =
                typeof block.content === 'string' ? block.content : truncate(safeStringify(block.content) ?? '', 2000)
            toolResults.push({
                role: 'tool',
